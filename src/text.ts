...
export function handleTextNode(textNode: Text, context: TraversalContext): void {
    ...
    while (true) {
        const addTextSpanForLineRange = (): void => {
            ...
            textSpan.setAttribute('x', (lineRectangle.x - context.options.captureArea.x).toString())
            textSpan.setAttribute('y', (lineRectangle.bottom - context.options.captureArea.y).toString()) // intentionally bottom because of dominant-baseline setting
