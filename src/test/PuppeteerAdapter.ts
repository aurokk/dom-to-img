...
function getBodyData(response: Puppeteer.HTTPResponse, buffer: Buffer): { body: string; isBinary: boolean } {
...
interface PollyRequestArguments {
  requestArguments: { request: Puppeteer.HTTPRequest }
}
...
export class PuppeteerAdapter extends PollyAdapter {
   ...
   */
  private pendingRequests = new Map<Puppeteer.HTTPRequest, { respond: (response: Puppeteer.HTTPResponse) => Promise<void> }>()
  ...
  private passThroughRequests = new Map<
    ...
    { responsePromise: Promise<Puppeteer.HTTPResponse>; respond: (response: Puppeteer.HTTPResponse) => void }
  >()
  ...
  public async passthroughRequest(pollyRequest: PollyRequest): Promise<PollyResponse> {
    ...
    let respond: (response: Puppeteer.HTTPResponse) => void
    const responsePromise = new Promise<Puppeteer.HTTPResponse>(resolve => (respond = resolve))
  ...
  public onRequest({
    ...
    const respond = async (response: Puppeteer.HTTPResponse): Promise<void> => {
      let bodyData = { isBinary: false, body: '' }
      if (response.status() < 300 || response.status() >= 400) {
        let buffer = await response.buffer()
        // No idea why, but CDP/Puppeteer report the body as empty for this request.
